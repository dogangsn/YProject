...
*/; (function ($, window, document, undefined) {
   ...
   })(); var defaults = { listNodeName: 'ol', itemNodeName: 'li', rootClass: 'dd', listClass: 'dd-list', itemClass: 'dd-item', dragClass: 'dd-dragel', handleClass: 'dd-handle', collapsedClass: 'dd-collapsed', placeClass: 'dd-placeholder', noDragClass: 'dd-nodrag', emptyClass: 'dd-empty', expandBtnHTML: '<button data-action="expand" type="button">Expand</button>', collapseBtnHTML: '<button data-action="collapse" type="button">Collapse</button>', group: 0, maxDepth: 5, threshold: 20, inputField: '<input type="text" placeholder="Text" class="inputField">', addRowAboveButton: '<a title="Add Row Above" class="arrow-up pull-right" data-action="addRowAbove"></a>', addRowBelowButton: '<a title="Add Row Below" class="arrow-down pull-right" data-action="addRowBelow"></a>', removeRowButton: '<a title="Remove Row" class="removeButton pull-right" data-action="removeRow">X</a>' }; function Plugin(element, options) { this.w = $(document); this.el = $(element); this.options = $.extend({}, defaults, options); this.init(); }
   Plugin.prototype = {
      init: function () {
         var list = this; list.reset(); list.el.data('nestable-group', this.options.group); list.placeEl = $('<div class="' + list.options.placeClass + '"/>'); $.each(this.el.find(list.options.itemNodeName), function (k, el) { list.setParent($(el)); }); list.el.on('click', 'button', function (e) {
            ...
            if (list.dragEl) { return; }
            var target = $(e.currentTarget), action = target.data('action'), item = target.parent().parent(list.options.itemNodeName); if (action === 'addRowAbove') { list.addRowAbove(list, item); }
            if (action === 'addRowBelow') { list.addRowBelow(list, item); }
            if (action === 'removeRow') { item.remove(); list.el.trigger('change'); }
         ...
         }; var onMoveEvent = function (e) { if (list.dragEl) { e.preventDefault(); list.dragMove(e.touches ? e.touches[0] : e); } }; var onEndEvent = function (e) { if (list.dragEl) { e.preventDefault(); list.dragStop(e.touches ? e.touches[0] : e); } }; if (hasTouch) { list.el[0].addEventListener('touchstart', onStartEvent, false); window.addEventListener('touchmove', onMoveEvent, false); window.addEventListener('touchend', onEndEvent, false); window.addEventListener('touchcancel', onEndEvent, false); }
         ...
         var nestableList = list.el.attr("id"); var idValue = parseInt($("#" + nestableList + " ol > li").length) + 1; var rowHtml = '<li class="dd-item dd3-item" data-id="' + idValue + '">' +
            ...
            '</div>' +
            '</li>'; rowHtml = $(rowHtml); $(rowHtml).insertBefore(item); list.el.trigger('change');
         ...
         var nestableList = list.el.attr("id"); var idValue = parseInt($("#" + nestableList + " ol > li").length) + 1; var rowHtml = '<li class="dd-item dd3-item" data-id="' + idValue + '">' +
            ...
            this.options.addRowBelowButton +
            '</div>' +
            '</li>'; rowHtml = $(rowHtml); $(rowHtml).insertAfter(item); list.el.trigger('change');
      }, serializeText: function () {
         var data, depth = 0, list = this; step = function (level, depth) {
            var array = [], items = level.children(list.options.itemNodeName); items.each(function () {
               var li = $(this), item = {}, sub = li.children(list.options.listNodeName); item["text"] = li.children(".dd3-content").children(".inputField").val(); if (sub.length) { item.children = step(sub, depth + 1); }
               array.push(item);
            }); return array;
         }; data = step(list.el.find(list.options.listNodeName).first(), depth); return data;
         ...
         var data, depth = 0, list = this; step = function (level, depth) {
            var array = [], items = level.children(list.options.itemNodeName); items.each(function () {
               var li = $(this), item = $.extend({}, li.data()), sub = li.children(list.options.listNodeName); item["text"] = li.children(".dd3-content").children(".inputField").val(); if (sub.length) { item.children = step(sub, depth + 1); }
               array.push(item);
            }); return array;
         }; data = step(list.el.find(list.options.listNodeName).first(), depth); return data;
      }
   }; $.fn.nestable = function (params) { var lists = this, retval = this; lists.each(function () { var plugin = $(this).data("nestable"); if (!plugin) { $(this).data("nestable", new Plugin(this, params)); $(this).data("nestable-id", new Date().getTime()); } else { if (typeof params === 'string' && typeof plugin[params] === 'function') { retval = plugin[params](); } } }); return retval || lists; };
